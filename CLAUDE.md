...
**Nao e um projeto de software tradicional** — e um framework de conhecimento/metodologia construido para Claude Code, composto por agentes Markdown, slash commands e templates.
...
| **play-insights** | Inteligencia Google Play (VOC, vitals, recomendacoes, replies) |
...
| `/pf-doctor` | Validar instalacao e consistencia | (terminal) |
| `/play-insights` | Pipeline Play Store (ingest, analise, relatorios, replies) | `docs/discovery/`, `docs/research/`, `.productflow/memory/` |

> **Nota**: Os comandos `/pf-review`, `/pf-status`, `/pf-init` e `/pf-doctor` usam prefixo `pf-` para evitar conflito com comandos built-in do Claude Code.

### Arquitetura Chave: Separacao Agentes vs Commands

Os agentes (`.claude/agents/`) sao **somente leitura** — nao possuem ferramentas Write/Edit. Eles preparam e retornam conteudo. Os slash commands (`.claude/commands/`) sao responsaveis por **persistir os artefatos em disco** nos diretorios corretos. Ao modificar o fluxo, respeite essa separacao.
...
/play-insights ingest -> /play-insights analyze -> /play-insights report --mode daily
                                                                       |
                                                         /play-insights report --mode weekly
```
...
```bash
# Validar estrutura e consistencia do ProductFlow
python scripts/validate_productflow.py          # todas as validacoes
python scripts/validate_productflow.py agentes   # apenas agentes
python scripts/validate_productflow.py comandos  # apenas comandos
python scripts/validate_productflow.py seguranca # apenas seguranca

# Reinstalar/atualizar ProductFlow (PowerShell - cria backup automatico)
